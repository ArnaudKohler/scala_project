...
class TopologicalSortingTest extends AnyFlatSpec {
    "A TopologicalSorting" should "be able to find a result in a graph" in {
        ...
        val result = TopologicalSorting.topologicalSort(u) match {
            case Left(errMsg) => errMsg
            case Right(sortedNodes) => sortedNodes
        }
        val expectedResult = List(n1, n3, n2, n4, n5)
        assert(result == expectedResult)
    }

    it should "be able to return error message if there is a cycle" in {
        val graph = DiGraph.empty[Int, UnweightedEdge[Int]]
        val n1 = 1
        val n2 = 2
        val n3 = 3
        ...
        val expectedResult = "Error, graph contains a cycle"
        assert(result == expectedResult)
    }
}